\achapter{15}{Quotient Spaces}\label{sec:quotients}
...
%If $\sim$ is an equivalence relation on a set $X$, we can partition $X$ into a union of disjoint equivalence classes $[x] = \{y \in X \mid y \sim x\}$. There is a natural map $q$ from $X$ to the space of equivalence classes given by $q(x) = [x]$. 
...
\item If $p: X \to Y$ is a quotient map, then the space $Y$ is a \textbf{quotient space}\index{quotient space}. 
\end{enumerate}
\end{definition}
...
\end{center}
\end{figure}
%[trim=1.8cm 4.0cm 1.9cm 4.7cm, clip]
%\includegraphics[trim=left bottom right top, clip]{file}	
...
%\resizebox{!}{1.25in}{\includegraphics[trim=1.8cm 4.0cm 1.25cm 4.7cm, clip]{Quotient_torus_1.png}} \hspace{0.75in} \resizebox{!}{1.25in}{\includegraphics[trim=2.75cm 2.1cm 2.0cm 3.5cm, clip]{Quotient_torus_2.png}} 
...
%	\item  All points in the interior of the square are identified only with themselves. However, the left side and right side are identified with each other, as are the top and bottom -- both in the same direction as indicated at left in Figure \ref{F:Quotient_torus}. So if we first roll up the square to identify the left and right sides, we obtain a cylinder. The top and bottom of the square become the top and bottom of the cylinder. Now bend the cylinder around to identify the top and bottom, resulting in the torus as shown at right in Figure \ref{F:Quotient_torus}. 
%\begin{figure}[h]
%\begin{center}
%\resizebox{!}{1.25in}{\includegraphics[trim=1.8cm 4.0cm 1.25cm 4.7cm, clip]{Quotient_torus_1.png}} \hspace{0.75in} \resizebox{!}{1.25in}{\includegraphics[trim=2.75cm 2.1cm 2.0cm 3.5cm, clip]{Quotient_torus_2.png}} 
%\caption{A torus as the identification space $X/\ssim$.} 
...
%\includegraphics[trim=left bottom right top, clip]{file}	
	...
	\item  All points in the interior of the square are identified only with themselves, but the boundaries points are all identified together as indicated at left in Figure \ref{F:Quotient_sphere}. So the boundary collapses to a point. Think of taking the square and pinching all of the sides together. What is left looks like a sphere, with the sides all identified with a single point on the sphere as shown at right in Figure \ref{F:Quotient_sphere}. %An illustration of this can be seen at \url{https://upload.wikimedia.org/wikipedia/commons/4/44/Disk_to_Sphere_using_Quotient_Space.gif}.
\begin{figure}[h]
\begin{center}
...
\begin{example}[Continued.] We return to the example with $X = \R$ under the standard topology with equivalence relation  $\sim$ defined by $x \sim y$ if $x - y \in \Z$. Our goal is to show that $\R/\ssim$ is homeomorphic to the circle $Y = S^1$. 
...
%is the 2-disc and A=?D2 (the boundary circle) then X/A=S2 (if we think of the centre of the disc as the North Pole then all the points in A are identified to get the South Pole. 
...
\begin{figure}[h]
\begin{center}
\resizebox{!}{1.25in}{\includegraphics[trim=3.55cm 3.75cm 3.55cm 3.0cm, clip]{projective_1.png}} \hspace{0.25in} \resizebox{!}{1.25in}{\includegraphics{projective_Mobius.eps}} \hspace{0.25in} \resizebox{!}{1.25in}{\includegraphics{projective_disk.eps}}
...
\begin{figure}[h]
\begin{center}
\resizebox{!}{1.25in}{\includegraphics{projective_3.eps}} \hspace{0.25in} \resizebox{!}{1.25in}{\includegraphics{projective_4.eps}}
\caption{Three perspectives of $\mathbb{R}P^2$.} 
...
Let $u = [(r,s)]$ and $v = [(x,y)]$ be in $X^*$. Suppose $\overline{f}(u) = \overline{f}(v)$. Then $r^2+s^2 = x^2+y^2$ from which it follows that $[(r,s)] = [(x,y)]$. Since $f$ is a surjection that preserves the relation, it follows that $\overline{f}$ is also a surjection.
